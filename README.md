...
### Generated with Codex using this Skill

The following two applications were generated by Codex from natural-language requirements after loading `$dify-workflow-dsl`, then imported into Dify and captured on the canvas. The downloadable YAML files are sanitized public copies; model, plugin, and dataset bindings must be reconnected in the target workspace.

#### Complex customer-service Chatflow

[Download the Chatflow DSL](examples/customer-service-chatflow.example.yml). It contains rule-based fast paths, intent and sentiment analysis, human handoff, enterprise knowledge retrieval, and structured responses.

![Dify canvas for the customer-service Chatflow generated by Codex using this Skill](docs/images/customer-service-chatflow-canvas.png)

#### Complex user-profile Workflow

[Download the Workflow DSL](examples/user-profile-workflow.example.yml). It analyzes text, images, and behavior events in parallel, then applies consent controls, evidence tracking, profile merging, and a quality gate.

![Dify canvas for the user-profile Workflow generated by Codex using this Skill](docs/images/user-profile-workflow-canvas.png)
